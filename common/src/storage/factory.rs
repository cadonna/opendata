...
impl StorageSemantics {
    ...
    /// Sets the segment extractor (RFC-0024) for SlateDB storage.
    ///
    /// SlateDB persists the extractor's name in the manifest and requires
    /// every subsequent open — including read-only `DbReader`s — to configure
    /// a matching extractor. Each system defines its own key layout and thus
    /// its own extractor. Ignored for in-memory storage.
